...
try {
    ...
    // Yöntem 1: Ayrı env değişkenleri (FIREBASE_PROJECT_ID + FIREBASE_CLIENT_EMAIL + FIREBASE_PRIVATE_KEY)
    if (process.env.FIREBASE_PROJECT_ID && process.env.FIREBASE_CLIENT_EMAIL && process.env.FIREBASE_PRIVATE_KEY) {
        serviceAccount = {
            type: 'service_account',
            project_id:   process.env.FIREBASE_PROJECT_ID,
            client_email: process.env.FIREBASE_CLIENT_EMAIL,
            // .env'deki \n kaçış karakterini gerçek satır sonuna çevir
            private_key:  process.env.FIREBASE_PRIVATE_KEY.replace(/\\n/g, '\n'),
        };
    }
    // Yöntem 2: Tek JSON string (eski yöntem, geriye dönük uyumluluk)
    ...
    if (serviceAccount) {
        ...
        console.warn('⚠️  Firebase yapılandırması bulunamadı. FCM push bildirimleri pasif.');
        console.warn('   .env\'e şunları ekle: FIREBASE_PROJECT_ID, FIREBASE_CLIENT_EMAIL, FIREBASE_PRIVATE_KEY');
        firebaseAdmin = null;
    }
} catch (e) {
    console.warn('⚠️  Firebase Admin başlatılamadı:', e.message);
    console.warn('   Paket eksikse: npm install firebase-admin');
    firebaseAdmin = null;
}
...
const ALLOWED_ORIGINS = [
    ...
    'http://localhost:3000',
    'http://localhost:5173',
    'http://localhost:5174',   // Vite ikinci port
...
// POST /api/users/fcm-token — Android native app FCM token güncelleme
// Android Kotlin: RetrofitInstance.api.updateFcmToken(mapOf("fcmToken" to token))
app.post('/api/users/fcm-token', authenticateToken, async (req, res) => {
    try {
        const token = req.body.fcmToken || req.body.token;
        if (!token) return res.status(400).json({ success: false, error: 'fcmToken zorunludur' });
        await dbRun(
            `INSERT INTO device_tokens (id, "userId", token, platform, "createdAt", "updatedAt", "isActive")
             VALUES ($1, $2, $3, 'android', NOW(), NOW(), TRUE)
             ON CONFLICT (token)
             DO UPDATE SET "userId" = $2, "isActive" = TRUE, "updatedAt" = NOW()`,
            [uuidv4(), req.user.id, token]
        );
        res.json({ success: true, message: 'FCM token kaydedildi' });
    } catch (e) {
        console.error('[fcm-token POST]', e);
        res.status(500).json({ success: false, error: 'Sunucu hatası' });
    }
});
